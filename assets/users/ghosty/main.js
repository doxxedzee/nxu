...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const songs = ["./assets/users/ghosty/main.mp3", "./assets/users/ghosty/b.mp3", "./assets/users/ghosty/a.mp3"];
    ...
    let playedSongs = [];
    let availableSongs = shuffleArray([...songs]); // Start with a shuffled version of the songs array

    function playNextRandomSong() {
        if (availableSongs.length === 0) {
            // All songs have been played, shuffle and start over
            availableSongs = shuffleArray([...songs]);
        }

        // Get the next song and remove it from the availableSongs array
        const nextSong = availableSongs.shift();
        playedSongs.push(nextSong); // Add to the played list (optional, for tracking)
    ...
    music.addEventListener('ended', playNextRandomSong);
    ...
    // Skip button plays next random song (ensures all are played before repeating)
    skipBtn.addEventListener('click', playNextRandomSong);
    ...
    async function fetchDiscordPresence() {
        const userId = '1079248097609257041'; // Replace with your Discord User ID
        ...
        try {
                    ...
                    if (activities.length > 0) {
                        ...
                        if (activity.name === 'Spotify') {
                            ...
                            const activityImage = activity.assets.large_image
                                ...
                                : './assets/activity-placeholder.png';
            ...
            discordStatusDiv.innerHTML = `
                ...
                <div class="text-container">
                    <a class="username" style="color: #FFD700; font-weight: bold; background-image: url('./assets/main-page/gold.gif'); background-size: cover;">${data.data.discord_user.username} <span class="role">[Owner]</span></a>
                    ${activityContent}
                </div>
            `;
        } catch (error) {
